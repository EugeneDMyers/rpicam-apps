 ...
 *
 * KIPR_object_detection.cpp - IMX500 object detection for various networks
 */
...
#define NAME "kipr_object_detection"
...
class ObjectDetection : public KIPRPostProcessingStage
{
public:
	ObjectDetection(RPiCamApp *app) : KIPRPostProcessingStage(app) {}
...
void ObjectDetection::Read(boost::property_tree::ptree const &params)
{
	...
	KIPRPostProcessingStage::Read(params);
}

void ObjectDetection::Configure()
{
	lt_objects_.clear();
	KIPRPostProcessingStage::Configure();
	if (!started_)
	{
		KIPRPostProcessingStage::ShowFwProgressBar();
		started_ = true;
	}
}
...
bool ObjectDetection::Process(CompletedRequestPtr &completed_request)
{
	...
	if (objects.size())
{
		LOG(1, "KIPR object_detect.results " << objects.size());
		completed_request->post_process_metadata.Set("object_detect.results", objects);
}

	return KIPRPostProcessingStage::Process(completed_request);
}
